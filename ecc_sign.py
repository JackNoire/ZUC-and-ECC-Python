...
class ECC:
    ...
    def sign(self, m):
        k = randint(1, self.n - 1)
        r, _ = self.multiply(k, self.Gx, self.Gy)
        s = (int(md5(m.to_bytes(64, 'little')).hexdigest(),16) - self.d*r) * inverse(k, self.n) % self.n
        return m, r, s
    
    def verify(self, m_r_s):
        m, r, s = m_r_s
        invS = inverse(s, self.n)
        x1, y1 = self.multiply(int(md5(m.to_bytes(64, 'little')).hexdigest(),16), self.Gx, self.Gy)
        x2, y2 = self.multiply(r, self.Qx, self.Qy)
        xu, yu = self.add(x1, y1, x2, self.p-y2)
        xu, yu = self.multiply(invS, xu, yu)
        return xu == r
...
if __name__ == "__main__":
    ...
    eccBlock = ECC()
    mrs = eccBlock.sign(msg)
    result = eccBlock.verify(mrs)
    print("原文：0x%x" % msg)
    print("签名：m:0x%x\n      r:0x%x\n      s:0x%x" % \
            (mrs[0], mrs[1], mrs[2]))
    print("验签结果：", result)